...
use glium::{backend::glutin::SimpleWindowBuilder, glutin::surface::WindowSurface, implement_vertex, uniform, winit::{application::ApplicationHandler, event::{ElementState, StartCause, WindowEvent}, event_loop::EventLoop, keyboard::{KeyCode, PhysicalKey}, window::Window}, Display, DrawParameters, IndexBuffer, Program, Surface, VertexBuffer};

use crate::{consts::{CANVAS_INDICES, MANDELBROT_CANVAS_POSITIONS, MAT4_ID}, info_display, shaders::{CANVAS_VERT, JULIA_FRAG, MANDELBROT_FRAG}};
...
impl ApplicationHandler for App<'_> {
    ...
    fn new_events(&mut self, _event_loop: &glium::winit::event_loop::ActiveEventLoop, cause: glium::winit::event::StartCause) {
...
impl App<'_> {
    pub fn new(event_loop : &EventLoop<()>)->Result<Self,Box<dyn Error>>{
        let (window, display) = SimpleWindowBuilder::new().with_title("Co's complex fractal").build(event_loop);
        ...
        let canvas_v_buff = VertexBuffer::new(&display, &MANDELBROT_CANVAS_POSITIONS)?;
        let canvas_indices = IndexBuffer::new(&display,glium::index::PrimitiveType::TriangleFan, &CANVAS_INDICES)?;
    ...
    fn draw(&self)->Result<(),Box<dyn Error>>{
        ...
        if self.draw_mandel{
            ...
            frame.draw(&self.canvas_v_buff, &self.canvas_indices, &self.julia_prog, &uni, &self.draw_params)?;
        }
